package multimodule.activty.service;

import multimodule.activty.model.Values;
import java.io.*;
import java.util.*;
...
public class Operations
{
		...
		for(int i=0; i<words.size();i++){
			...
			for(Values sWord:temp){
				if(sWord.value1.contains(find) || sWord.value2.contains(find)){		
					System.out.println("("+i+","+k+")("+sWord.value1+","+sWord.value2+")->");
					System.out.println(find+"'s Occurence/s: ");
					int count =0;
					match = true;
	...
	public void edit(int row, int col){
		try{	
				...
				System.out.println("Enter New Value1: ");
				temp.value1 = scan.next();
				System.out.println("Enter New Value2: ");				
				temp.value2 = scan.next();
